...
const filePath = {
  winURL: {
    ...
    production: `file://${join(__dirname, "dist", "electron", 'renderer', 'index.html')}`
  },
  loadingURL: {
    development: `http://localhost:${process.env.PORT}/loader.html`,
    production: `file://${join(__dirname, "dist", "electron", 'renderer', 'loader.html')}`
  },
  getUrl: (filename?: string) => {
    if (env === 'development') {
      return `http://localhost:${process.env.PORT}${filename?`/#/${filename}`:''}`
    }
    return `file://${join(__dirname, "dist", 'renderer', `index.html${filename?`/#/${filename}`:''}`)}`
  },
  settingURL: {
    development: `http://localhost:${process.env.PORT}/setting.html`,
    production: `file://${join(__dirname, "dist", "electron", 'renderer', 'setting.html')}`
  },
  __static: {
  ...
  getPreloadFile(fileName: string) {
    if (env !== 'development') {
      return join(__dirname, "dist", "preload", `${fileName}.js`)
    }
