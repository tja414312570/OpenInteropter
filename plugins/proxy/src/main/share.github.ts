...
class DeltaProcessor {
  ...
  applyDeltaToObject(targetObject: Delta, delta: Delta) {
    return traverseObject(
      ...
      (root: any) => applyOperation(root, delta)
    ).__root;
  }
}
// 辅助函数：解析路径
...
function applyOperation(obj: any, delta: Delta) {
  ...
  switch (delta.op) {
    ...
    case "add":
      Array.isArray(obj)
        ? obj.splice(lastKey, 0, delta.value)
    ...
    case "append":
      if (typeof obj[lastKey] === "string") {
        ...
        for (let o in delta.value) obj[lastKey][o] = delta.value[o];
      } else {
        obj[lastKey] = delta.value;
      }
      break;
    ...
    try {
      ...
      if (event.event !== "ping") {
        ...
        if (event.event === "delta_encoding") {
          if (
            originData === StreamEncoding.V1 ||
            originData === StreamEncoding.V1_TEST
          ) {
        ...
        // 处理 "delta" 事件
          ...
          if (!this.deltaProcessor) {
            // 检查是否有增量处理器
            this.throwError(
              "[completion-delta] delta 事件在 delta_encoding 之前触发"
            );
            return;
          }
          // 将增量应用到对象上
