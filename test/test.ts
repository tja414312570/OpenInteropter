...
if (PLATFORM === 'win32') {
    pythonDownloadUrl = 'https://www.python.org/ftp/python/3.x.x/python-3.x.x-embed-amd64.zip';  // Windows 嵌入式 Python
    downloadPath = path.join(DOWNLOAD_DIR, 'python_embed.zip');
} else if (PLATFORM === 'darwin') {
    //  https://repo.anaconda.com/miniconda/Miniconda3-latest-MacOSX-x86_64.sh
    pythonDownloadUrl = 'https://repo.anaconda.com/miniconda/Miniconda3-latest-MacOSX-x86_64.sh';  // macOS Miniconda
    downloadPath = path.join(DOWNLOAD_DIR, 'miniconda.sh');
} else if (PLATFORM === 'linux') {
    pythonDownloadUrl = 'https://repo.anaconda.com/miniconda/Miniconda3-latest-Linux-x86_64.sh';  // Linux Miniconda
    downloadPath = path.join(DOWNLOAD_DIR, 'miniconda.sh');
} else {
...
function installPython() {
    if (PLATFORM === 'win32') {
        ...
        const installProcess = spawn('bash', [downloadPath, '-b', '-p', INSTALL_DIR, '-f']);
...
function setupPipAndPackages() {
    const pythonExecutable = PLATFORM === 'win32' ? path.join(INSTALL_DIR, 'python.exe') : path.join(INSTALL_DIR, 'bin', 'python');
...
function installPackages() {
    ...
    const pipInstall = spawn(pythonExecutable, ['-m', 'pip', 'install', ...packages]);
    pipInstall.stdout.on('data', (data) => console.log(data.toString()));
    pipInstall.stderr.on('data', (data) => console.error(data.toString()));
    pipInstall.on('close', (code) => {
        if (code === 0) {
